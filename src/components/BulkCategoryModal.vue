    ...
    <div class="modal-content bulk-modal">
      ...
      <div v-if="step === 1" class="step-content">
        <div class="template-section">
          <p>
            Download the template, fill it with your categories, and upload it
            back.
          </p>
          ...
          >
            ...
            <div class="item-actions">
              <button
                ...
                title="Update existing record with this description"
              >
                Update Existing
              </button>
              <button
      ...
      <div class="modal-footer">
        ...
        >
          {{ loading ? "Processing..." : `Add ${validCount} Categories` }}
        </button>
      </div>
    </div>
  </div>
...
function processParsedData(data) {
  const existingCategories = categoryStore.categories;
  ...
  parsedData.value = data
    .map((row) => {
      ...
      const fuzzyMatch = existingCategories.find((c) =>
        isAlmostMatch(c.name, name),
      );
      if (fuzzyMatch) {
        return {
          name,
      ...
      return { name, description, status: "valid", originalName: name };
    })
    .filter(Boolean);

  if (parsedData.value.length === 0) {
    dialogStore.error("No valid categories found in the file.");
    return;
  }
...
async function submitBulk() {
  ...
  try {
    ...
    for (const item of toAdd) {
      try {
        if (item.isUpdate && item.matchedWith) {
          await categoryStore.updateCategory(item.matchedWith.id, {
            name: item.matchedWith.name,
            description: item.description || item.matchedWith.description,
          });
        } else {
    ...
    dialogStore.success(`Successfully processed ${successCount} categories`);
    emit("success");
    emit("close");
  } catch (error) {
